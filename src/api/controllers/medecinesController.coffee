...
module.exports.getAllPharmaMedecines = (req, res) ->
	...
	db.query
		sql: 'select m.*, pm.*, p.name as provider from medecines m inner join `pharma-medecines` pm on m.id = pm.medecine inner join providers p on pm.provider = p.id where pm.pharmacy = ? order by pm.id desc'
